...
public class PanelGestionEstudiante extends JPanel {
	...
	private void nuevo() {
		...
		//Muy importante esta línea para poder guardar nuevos registros, tenemos que crear un nuevo objeto Estudiante
	...
	private void guardar() throws ParseException {
		...
		if(actual.getId() == 0) {
			EstudianteControlador.getInstancia().persist(this.actual);
		}
		else {
			EstudianteControlador.getInstancia().merge(this.actual);
		}
